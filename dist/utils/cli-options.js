 ...
 * ```
 */
export function addStandardOption(command, optionName) {
 ...
 * ```
 */
export function addStandardOptions(command, optionNames) {
 ...
 */
export function isStandardOption(name) {
    return STANDARD_OPTION_NAMES.includes(name);
}
/**
 ...
 */
export function hasOption(options, optionName) {
 ...
 * ```
 */
export function validateOptionConflicts(commandName, options) {
    ...
    for (const rule of rules) {
        ...
        const setOptions = rule.exclusive.filter((optName) => isOptionSet(options[optName]));
        ...
        if (setOptions.length > 1) {
            ...
            errors.push(`Options ${flagNames.join(' and ')} cannot be used together: ${rule.message}`);
        }
    }
    return {
 ...
 */
export function getConflictRules(commandName) {
 ...
 */
export function hasConflictRules(commandName) {
    const rules = OPTION_CONFLICTS[commandName];

...
import { isTelemetryEnabled, getCurrentTraceId, getCurrentSpanId, startTaskSpan, endSpanSuccess, endSpanError, } from './telemetry.js';
/**
 ...
 * ```
 */
export function startTiming(operationName, attributes) {
    const ctx = {
        startTime: performance.now(),
    };
    // Create span if telemetry is enabled (regardless of verbose mode)
    if (isTelemetryEnabled()) {
        ctx.span = startTaskSpan(operationName, attributes);
    }
 ...
 * ```
 */
export function startTraceTiming(operationName, attributes) {
    const ctx = {
        startTime: performance.now(),
    };

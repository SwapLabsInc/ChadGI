 ...
 */
export const SESSION_STATS_SCHEMA = {
    name: 'SessionStats',
 ...
 */
export const TASK_METRICS_SCHEMA = {
    name: 'TaskMetrics',
    additionalProperties: true,
    fields: {
        issue_number: {
 ...
 */
export const METRICS_DATA_SCHEMA = {
    name: 'MetricsData',
    additionalProperties: true,
    fields: {
        version: {
 ...
 */
export const TASK_LOCK_DATA_SCHEMA = {
    name: 'TaskLockData',
    additionalProperties: true,
    fields: {
        issue_number: {
 ...
 */
export const PROGRESS_DATA_SCHEMA = {
    name: 'ProgressData',
    additionalProperties: true,
    fields: {
        status: {
 ...
 */
export const PAUSE_LOCK_DATA_SCHEMA = {
    name: 'PauseLockData',
    additionalProperties: true,
    fields: {
        paused_at: {
 ...
 */
export const APPROVAL_LOCK_DATA_SCHEMA = {
 ...
 */
function validateField(value, constraint, path, errors, recover) {
    let recovered = false;
...
export function validateSchema(data, schema, options = {}) {
    ...
    return {
        valid,
        data: valid || hasRecoveries ? validatedData : undefined,
        errors,
        hasRecoveries,
    };
}
 ...
 */
export function validateArray(data, itemSchema, options = {}) {
    const { recover = true, filePath } = options;
    const errors = [];
    ...
    if (errors.length > 0 && isVerbose()) {
        ...
        if (skippedCount > 0) {
            process.stderr.write(`[DEBUG] Skipped ${skippedCount} invalid item(s) from ${itemSchema.name} array${fileContext}\n`);
        }
    }
    return {

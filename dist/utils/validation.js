 ...
 */
export function validateNumeric(value, optionName, constraint) {
    // Resolve predefined constraint name to configuration
    const config = typeof constraint === 'string'
 ...
 * ```
 */
export function createNumericParser(optionName, constraint) {
    return (value) => {
 ...
 */
export function validateNumericOptions(options, constraints) {
    const errors = [];
 ...
 */
export function formatConstraintBounds(constraint) {
    const config = typeof constraint === 'string'
        ? NUMERIC_CONSTRAINTS[constraint]
        : constraint;
    const parts = [];

import { existsSync, readFileSync, readdirSync, statSync, unlinkSync, watchFile, unwatchFile, } from 'fs';
 ...
 */
function resolveLogPath(chadgiDir, configContent) {
    const logFile = parseYamlNested(configContent, 'output', 'log_file') || DEFAULT_LOG_FILE;
 ...
 * - 2024-01-15T10:30:00Z INFO message
 */
function parsePlainTextLogLine(line) {
 ...
 */
function formatLogEntry(entry, showFullTimestamp = false) {
 ...
 */
function printLogs(entries, total, options, sinceDate, levelFilter, taskFilter, grepPattern) {
 ...
 */
function findLogFiles(chadgiDir, configContent) {
    const logFile = parseYamlNested(configContent, 'output', 'log_file') || DEFAULT_LOG_FILE;
 ...
 */
async function followLogs(logFilePath, options) {
    let running = true;
    let lastPosition = 0;
    let lastEntries = [];
...
export async function logsClear(options = {}) {
    ...
    if (!options.yes && !options.json) {
        ...
        console.log('');
        const confirmed = await promptConfirmation(`${colors.yellow}Delete ${filesToDelete.length} log file(s)?${colors.reset}`);
        if (!confirmed) {

...
import { parseYamlValue, parseYamlNested, parseEnvOverrides, loadConfigWithEnv, validateConfigLogic, DEFAULT_ENV_PREFIX, } from './utils/config.js';
...
import { withCommand, withDirectory, } from './utils/index.js';
...
function checkClaudeAuthentication() {
    try {
        ...
        try {
            ...
            const errOutput = String(authErr.stderr || '');
...
function resolveConfigPathRelative(baseDir, configPath) {
    if (!configPath)
        return '';
 ...
 */
async function validateHandler(ctx) {
 ...
 */
export const validateMiddleware = withCommand([withDirectory], validateHandler);

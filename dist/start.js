...
function tryCreateChadgiUser() {
    try {
        ...
        console.log(`${colors.yellow}Could not create 'chadgi' user: ${err.message}${colors.reset}\n`);
        return false;
    }
}
...
function runRepoTask(repoPath, configPath, options) {
...
async function startWorkspace(options) {
    ...
    if (options.repo) {
        reposToProcess = enabledRepos.filter(([name]) => name === options.repo || name.endsWith('/' + options.repo));

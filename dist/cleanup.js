 ...
 */
function getOrphanedBranches(repo, branchPrefix) {
 ...
 * Delete local branch
 */
function deleteLocalBranch(branch, dryRun) {
    if (dryRun) {
        return true;
    }
    try {
 ...
 */
function getOldDiagnostics(chadgiDir, retentionDays) {
 ...
 */
function deleteDiagnostic(chadgiDir, name, dryRun) {
    if (dryRun) {
        return true;
    }
    try {
 ...
 */
function getOldLogFiles(chadgiDir, configContent) {
 ...
 * Delete log file
 */
function deleteLogFile(chadgiDir, configContent, name, dryRun) {
    if (dryRun) {
        return true;
    }
    try {
...
export async function cleanup(options = {}) {
    ...
    let orphanedBranches = { local: [], remote: [] };
    let oldDiagnostics = [];
    let oldLogs = [];
    ...
    const totalItems = orphanedBranches.local.length +
    ...
    if (!dryRun && !skipConfirmation) {
        const confirmed = await promptConfirmation(`${colors.yellow}This will permanently delete ${totalItems} item(s). Continue?${colors.reset}`);
        if (!confirmed) {

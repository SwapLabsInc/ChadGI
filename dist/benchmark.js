...
function loadCustomTasks(chadgiDir) {
...
function loadBenchmarkHistory(chadgiDir) {
...
function saveBenchmarkHistory(chadgiDir, history) {
    const historyFile = join(chadgiDir, 'benchmark-results.json');
    history.lastUpdated = new Date().toISOString();
    writeFileSync(historyFile, JSON.stringify(history, null, 2));
}
// Run a single benchmark task using Claude
async function runBenchmarkTask(task, config, dryRun = false) {
    ...
    try {
        ...
        const result = execSync(`claude ${args.join(' ')} "${task.prompt.replace(/"/g, '\\"')}"`, {
            encoding: 'utf-8',
...
async function runBenchmark(tasks, config, mode, dryRun = false, jsonMode = false) {
...
function compareBenchmarks(baseline, comparison) {
...
function generateMarkdownReport(result, comparison) {
...
function printBenchmarkResults(result, comparison) {
...
function printAvailableTasks(quickTasks, fullTasks, customTasks) {
...
export function initBenchmarks(chadgiDir) {
    const benchmarksDir = join(chadgiDir, 'benchmarks');
    if (!existsSync(benchmarksDir)) {

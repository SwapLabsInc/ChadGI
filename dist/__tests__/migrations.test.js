...
jest.unstable_mockModule('fs', () => ({
    ...
    writeFileSync: jest.fn((path, data) => vol.writeFileSync(path, data)),
    mkdirSync: jest.fn((path, options) => vol.mkdirSync(path, options)),
    copyFileSync: jest.fn((src, dest) => vol.copyFileSync(src, dest)),
    readdirSync: jest.fn((path) => vol.readdirSync(path)),
}));
// Import modules after mocking
const { parseVersion, compareVersions, isValidVersion, parseYamlToObject, objectToYaml, getConfigVersion, checkMigrations, createBackup, getLatestBackup, restoreFromBackup, loadMigrationHistory, saveMigrationHistory, addMigrationHistoryEntry, runMigrations, previewMigrations, CURRENT_CONFIG_VERSION, DEFAULT_CONFIG_VERSION, MIGRATION_HISTORY_FILE, BACKUP_DIR, MIGRATIONS, } = await import('../migrations/index.js');

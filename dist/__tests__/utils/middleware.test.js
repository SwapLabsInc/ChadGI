...
const { EXIT_CODES, composeMiddleware, withTiming, withDirectory, withDirectoryValidation, withConfig, withJsonOutput, withErrorHandler, withCommand, standardDirectoryMiddleware, standardConfigMiddleware, withDirectoryCommand, withConfigCommand, conditionalMiddleware, passthrough, } = await import('../../utils/middleware.js');
const { createCoreContext, } = await import('../../utils/context.js');
...
describe('middleware', () => {
    ...
    describe('composeMiddleware', () => {
        ...
        it('should propagate errors through the chain', async () => {
            const errorMiddleware = async (_ctx, _next) => {
    ...
    describe('withTiming', () => {
        it('should add startTime to context', async () => {
            const ctx = createCoreContext({ options: {} });
            let receivedStartTime;
            await withTiming(ctx, async (timedCtx) => {
    ...
    describe('withDirectory', () => {
        it('should resolve default chadgiDir when no config option', async () => {
            ...
            let resolvedDir;
            await withDirectory(ctx, async (dirCtx) => {
...
`;
            ...
            let loadedGithub;
            let loadedBranch;
            await withConfig(ctx, async (configCtx) => {
    ...
    describe('withErrorHandler', () => {
        ...
        it('should catch errors and exit with code 1', async () => {
            const ctx = createCoreContext({ options: {} });
            await expect(withErrorHandler(ctx, async () => {
                throw new Error('Test error');
    ...
    describe('withCommand', () => {
        it('should execute handler with full middleware chain', async () => {
            ...
            let handlerCalled = false;
            const command = withCommand([withDirectory, withDirectoryValidation], async (ctx) => {
                handlerCalled = true;
                return { success: true };
            });
        ...
        it('should automatically add timing middleware', async () => {
            ...
            const command = withCommand([], async (ctx) => {
        ...
        it('should handle errors from handler', async () => {
            const command = withCommand([], async () => {
    ...
    describe('withDirectoryCommand', () => {
        it('should create a command with directory middleware', async () => {
            ...
            let receivedChadgiDir;
            const command = withDirectoryCommand(async (ctx) => {
    ...
    describe('withConfigCommand', () => {
        it('should create a command with config middleware', async () => {
            const command = withConfigCommand(async (ctx) => {
    ...
    describe('conditionalMiddleware', () => {
        it('should run middleware when option is not set', async () => {
            ...
            const conditional = conditionalMiddleware('skipIt', testMiddleware);
            const ctx = createCoreContext({ options: {} });
        ...
        it('should skip middleware when option is set', async () => {
            ...
            const conditional = conditionalMiddleware('skipIt', testMiddleware);
            const ctx = createCoreContext({ options: { skipIt: true } });
            await conditional(ctx, async () => { });
        ...
        it('should invert condition when invert=true', async () => {
            ...
            const conditional = conditionalMiddleware('runIt', testMiddleware, true // invert
            );
            const ctx = createCoreContext({ options: { runIt: true } });
            await conditional(ctx, async () => { });
            expect(middlewareRan).toBe(true);
        });
    });
    describe('passthrough', () => {
        ...
        it('should pass through result from handler', async () => {
            const ctx = createCoreContext({ options: {} });
            const result = await passthrough(ctx, async () => {
                return { data: 'test' };
            });
            expect(result).toEqual({ data: 'test' });
        });

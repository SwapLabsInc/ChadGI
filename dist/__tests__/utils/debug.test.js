...
jest.unstable_mockModule('../../utils/secrets.js', () => ({
    maskSecrets: jest.fn((text) => text.replace(/secret123/gi, '[REDACTED]')),
}));
// Import after mocking
const { ENV_VERBOSE, ENV_TRACE, PREFIX_DEBUG, PREFIX_TRACE, initDebugFromEnv, getDebugConfig, setVerbosityLevel, enableVerbose, enableTrace, disableDebug, resetDebugConfig, setTimestamps, setMaskSecretsInDebug, isVerbose, isTrace, debugLog, traceLog, startTiming, startTraceTiming, debugDecision, debugFileOp, traceApi, traceApiResponse, } = await import('../../utils/debug.js');
...
describe('startTiming', () => {
    ...
    it('should output start and end messages when verbose', () => {
        ...
        const startOutput = stderrSpy.mock.calls[0][0];
        const endOutput = stderrSpy.mock.calls[1][0];

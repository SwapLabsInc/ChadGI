 ...
 */
export function parseYamlToObject(content) {
    const result = {};
 ...
 */
export function objectToYaml(obj, indent = 0) {
    const lines = [];
 ...
 */
export function getConfigVersion(configOrPath) {
    let config;
 ...
 */
export function checkMigrations(configPath) {
...
export function getLatestBackup(configPath) {
    ...
    const configName = basename(configPath, '.yaml');
    const backups = files
        .filter((f) => f.startsWith(configName) && f.endsWith('.yaml'))
        .sort()
        .reverse();
    if (backups.length === 0) {
        return null;
 ...
 */
export function restoreFromBackup(configPath) {
 ...
 */
export function loadMigrationHistory(chadgiDir) {
 ...
 */
export function saveMigrationHistory(chadgiDir, history) {
    const historyPath = join(chadgiDir, MIGRATION_HISTORY_FILE);
 ...
 */
export function addMigrationHistoryEntry(chadgiDir, entry) {
 ...
 */
export function runMigrations(configPath, options = {}) {
 ...
 */
export function previewMigrations(configPath) {
    const checkResult = checkMigrations(configPath);
    return {

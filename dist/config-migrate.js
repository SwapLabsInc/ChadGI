...
import { checkMigrations, runMigrations, previewMigrations, restoreFromBackup, getLatestBackup, loadMigrationHistory, DEFAULT_CONFIG_VERSION, parseYamlToObject, } from './migrations/index.js';
/**
 ...
 */
async function handleRollback(configPath, options) {
 ...
 */
export async function configMigrate(options = {}) {
 ...
 */
export function hasPendingMigrations(configPath) {
    if (!existsSync(configPath)) {
        return false;
    }
 ...
 */
export function getMigrationStatusMessage(configPath) {
    if (!existsSync(configPath)) {
        return null;
    }
    const check = checkMigrations(configPath);
 ...
 */
export async function printMigrationHistory(options = {}) {
    const cwd = process.cwd();
    const defaultConfigPath = join(cwd, '.chadgi', 'chadgi-config.yaml');
    const configPath = options.config ? resolve(options.config) : defaultConfigPath;
    const chadgiDir = dirname(configPath);

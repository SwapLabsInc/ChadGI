...
import { Section, Table, hyperlink } from './utils/textui.js';
...
function getProjectBoardMetadata(projectNumber, repoOwner) {
    try {
        // Get project ID
...
function moveItemToColumn(itemId, projectId, statusFieldId, optionId) {
    try {
...
export async function queueSkip(options) {
    ...
    try {
        const output = execSync(`gh project item-list ${projectNumber} --owner "${repoOwner}" --format json --limit 100`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
        const items = JSON.parse(output);
        const item = items.items?.find((i) => i.content?.type === 'Issue' &&
...
export async function queuePromote(options) {
    ...
    try {
        ...
        try {
            execSync(`gh issue edit ${options.issueNumber} --repo "${repo}" --add-label "${labelToAdd}"`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
            const result = {
                success: true,
                action: 'promote',
                issueNumber: options.issueNumber,
            ...
            const result = {
                ...
                action: 'promote',
                issueNumber: options.issueNumber,
                message: `Failed to add priority label: ${labelError.message}`,
            };
            if (options.json) {
                console.log(JSON.stringify(result, null, 2));
            }

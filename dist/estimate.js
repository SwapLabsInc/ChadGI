...
function getReadyTasks(repo, projectNumber, readyColumn) {
    ...
    try {
        ...
        for (const task of readyTasks) {
            try {
                const issueOutput = execSync(`gh issue view ${task.number} --repo "${repo}" --json labels,body`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
...
function calculateCategoryEstimates(tasks) {
    const categoryMap = {};
...
function calculateOverallEstimate(tasks) {
...
function getTaskEstimate(task, categoryEstimates, overallEstimate) {
    const category = task.category || 'uncategorized';
    let estimate = null;
    let confidence = 'low';
    ...
    let minCost, maxCost, expectedCost;
    let minDuration, maxDuration, expectedDuration;
    if (estimate && estimate.sampleSize >= 3) {
        ...
        // Use defaults
        const defaultCost = DEFAULT_ESTIMATES[category] || DEFAULT_ESTIMATES.uncategorized;
        const defaultDuration = DEFAULT_DURATION_ESTIMATES[category] || DEFAULT_DURATION_ESTIMATES.uncategorized;
...
function generateEstimate(readyTasks, categoryEstimates, overallEstimate, budget) {
    const taskEstimates = [];
...
export async function estimate(options = {}) {

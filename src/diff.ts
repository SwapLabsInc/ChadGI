...
function extractIssueNumber(branch: string, branchPrefix: string): number | null {
...
function findBranchForIssue(issueNumber: number, branchPrefix: string): string | null {
  try {
...
function getDiffStats(baseBranch: string, targetBranch: string): { filesChanged: number; insertions: number; deletions: number } {
  try {
...
function getChangedFiles(baseBranch: string, targetBranch: string): FileChange[] {
  ...
  try {
    ...
    const statusMap = new Map<string, { status: 'added' | 'modified' | 'deleted' | 'renamed'; oldPath?: string }>();
    ...
    for (const line of output.split('\n')) {
      ...
      if (parts.length >= 3) {
        ...
        const statusInfo = statusMap.get(file) || { status: 'modified' as const };
...
function getCommits(baseBranch: string, targetBranch: string): CommitInfo[] {
  const commits: CommitInfo[] = [];

  try {
    const output = execSync(
      `git log --format="%H|%h|%s|%an|%aI" "${baseBranch}..${targetBranch}"`,
      {
        encoding: 'utf-8',
        stdio: ['pipe', 'pipe', 'pipe'],
      }
...
function getFullDiff(baseBranch: string, targetBranch: string): string {
  try {
...
function getPrDiff(repo: string, prNumber: number): { diff: string; baseBranch: string; headBranch: string } | null {
  try {
...
function findDiffTool(): string | null {
...
function highlightDiff(diff: string, useExternalTool: boolean = true): string {
  if (!diff) return '';
...
function printStatView(result: DiffResult): void {
...
function printFilesView(result: DiffResult): void {
  console.log(`${colors.purple}${colors.bold}`);
  console.log('==========================================================');
...
function printFullDiff(result: DiffResult, diff: string): void {
  console.log(`${colors.purple}${colors.bold}`);
  console.log('==========================================================');
  console.log('                    CHADGI DIFF                           ');
  console.log('==========================================================');
...
// Main diff function
export async function diff(issueNumber?: number, options: DiffOptions = {}): Promise<void> {
  ...
  let resolvedIssueNumber: number | undefined;
...
function computeStatsFromFiles(files: FileChange[]): { filesChanged: number; insertions: number; deletions: number } {
  return {
...
export async function diffPr(prNumber: number, options: DiffOptions = {}): Promise<void> {

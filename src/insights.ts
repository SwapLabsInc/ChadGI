...
function printInsights(analysis: AnalysisResults, chadgiDir: string): void {
...
export function initMetricsFile(chadgiDir: string): void {
  const metricsFile = join(chadgiDir, 'chadgi-metrics.json');
  if (!existsSync(metricsFile)) {
...
export function addTaskMetric(chadgiDir: string, metric: TaskMetrics): void {
  const metricsFile = join(chadgiDir, 'chadgi-metrics.json');

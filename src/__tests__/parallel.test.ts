...
jest.unstable_mockModule('../utils/fileOps.js', () => ({
  atomicWriteJson: jest.fn((path: string, data: unknown) => {
    vol.writeFileSync(path, JSON.stringify(data, null, 2));
  }),
  atomicWriteFile: jest.fn((path: string, content: string) => {
    vol.writeFileSync(path, content);
  }),
}));

// Import types for testing
import type { ProgressData, ParallelWorkerTask, ParallelSessionProgress } from '../types/index.js';
...
describe('Parallel Task Processing', () => {
  ...
  describe('ParallelWorkerTask type', () => {
    it('should have correct structure for worker task data', () => {
      const workerTask: ParallelWorkerTask = {
        worker_id: 1,
  ...
  describe('ParallelSessionProgress type', () => {
    it('should track aggregate cost across workers', () => {
      const session: ParallelSessionProgress = {
        started_at: new Date().toISOString(),
        tasks_completed: 5,
        total_cost_usd: 0.25,
        active_workers: 3,
  ...
  describe('max_parallel_tasks setting', () => {
    ...
    it('should default to 1 when not specified', () => {
      const workspaceConfig = {
        ...
        settings: {
          auto_clone: false,
        } as { auto_clone: boolean; max_parallel_tasks?: number },
        created_at: '',
        updated_at: '',
      };
...
describe('Parallel Worker Status Tracking', () => {
  it('should support all valid worker statuses', () => {
    const statuses: Array<ParallelWorkerTask['status']> = ['idle', 'in_progress', 'completed', 'failed'];

    for (const status of statuses) {
      const worker: ParallelWorkerTask = {
        worker_id: 1,
        repo_name: 'test/repo',
        repo_path: '/test/repo',
        status,
...
describe('Cost Aggregation', () => {
  it('should correctly aggregate costs across workers', () => {
    const workers: ParallelWorkerTask[] = [

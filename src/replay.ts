...
function getMostRecentSession(sessions: SessionStats[]): SessionStats | null {
...
function issueExists(repo: string, issueNumber: number): boolean {
  try {
    execSync(
      `gh issue view ${issueNumber} --repo "${repo}" --json number`,
      { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'], timeout: 5000 }
    );
    return true;
  } catch {
...
function branchExistsRemote(branch: string): boolean {
  try {
...
function hasUncommittedChanges(branch: string): boolean {
  try {
...
function getBranchForIssue(issueNumber: number, branchPrefix: string): string {
...
function getFailedTasks(
  ...
  let sessionFilter: Set<number> | null = null;
  if (lastSessionOnly) {
  ...
  const sortedMetrics = failedMetrics.sort((a, b) =>
    new Date(b.started_at).getTime() - new Date(a.started_at).getTime()
  );

  for (const metric of sortedMetrics) {
    // Skip if already processed (take most recent failure)
    if (processedIssues.has(metric.issue_number)) continue;

    // Skip if filtering to last session and not in that session
    if (sessionFilter && !sessionFilter.has(metric.issue_number)) continue;
...
function moveIssueToReady(
  ...
  try {
    ...
    const item = items.items?.find((i: { content?: { type: string; number: number } }) =>
    ...
    if (!item) {
      ...
      try {
        execSync(
          `gh project item-add ${projectNumber} --owner "${repoOwner}" --url "${issueUrl}"`,
          { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }
        );
      } catch {
        return false;
      ...
      const newItemsOutput = execSync(
        `gh project item-list ${projectNumber} --owner "${repoOwner}" --format json --limit 100`,
        { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }
      );
      const newItems = JSON.parse(newItemsOutput);
      const newItem = newItems.items?.find((i: { content?: { type: string; number: number } }) =>
        i.content?.type === 'Issue' && i.content?.number === issueNumber
      );
    ...
    const readyOptionId = statusField.options?.find((o: { name: string }) => o.name === readyColumn)?.id;

    if (!readyOptionId) return false;

    // Move item to Ready column
    execSync(
      `gh project item-edit --project-id "${project.id}" --id "${item.id}" --field-id "${statusField.id}" --single-select-option-id "${readyOptionId}"`,
      { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }
    );
...
// Delete local branch
function deleteLocalBranch(branch: string): boolean {
  try {
...
function deleteRemoteBranch(branch: string): boolean {
  try {
...
function incrementRetryCount(chadgiDir: string, issueNumber: number): void {
  const metricsFile = join(chadgiDir, 'chadgi-metrics.json');
...
function printFailedTaskHistory(tasks: FailedTask[], repo: string): void {
  ...
  for (const task of tasks) {
    // Issue line
    console.log(
      `${colors.bold}#${task.issueNumber}${colors.reset} ${colors.red}[FAILED]${colors.reset}`
    );
...
export async function replay(
  ...
  if (!skipConfirmation && !dryRun && !options.json) {
    const confirmed = await promptConfirmation(
      `${colors.yellow}Replay issue #${issueNumberArg}?${colors.reset}`
    );
    if (!confirmed) {
...
export async function replayLast(options: ReplayOptions = {}): Promise<void> {
  const cwd = process.cwd();
  const defaultConfigPath = join(cwd, '.chadgi', 'chadgi-config.yaml');
  const configPath = options.config ? resolve(options.config) : defaultConfigPath;
  const chadgiDir = dirname(configPath);
...
export async function replayAllFailed(options: ReplayOptions = {}): Promise<void> {
  const cwd = process.cwd();
  const defaultConfigPath = join(cwd, '.chadgi', 'chadgi-config.yaml');
  const configPath = options.config ? resolve(options.config) : defaultConfigPath;
  const chadgiDir = dirname(configPath);
  ...
  if (!skipConfirmation && !dryRun && !options.json) {
    const confirmed = await promptConfirmation(
      `${colors.yellow}Replay all ${failedTasks.length} failed task${failedTasks.length !== 1 ? 's' : ''}?${colors.reset}`
    );
    if (!confirmed) {
      console.log(`${colors.dim}Replay cancelled.${colors.reset}`);
      return;

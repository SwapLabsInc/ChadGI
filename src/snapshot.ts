...
function getSnapshotsDir(chadgiDir: string): string {
  return join(chadgiDir, 'snapshots');
}

// Ensure snapshots directory exists
function ensureSnapshotsDir(chadgiDir: string): string {
...
function getSnapshotPath(snapshotsDir: string, name: string): string {
  const sanitized = sanitizeSnapshotName(name);
  return join(snapshotsDir, `${sanitized}.json`);
}

// Load all snapshots
function loadAllSnapshots(snapshotsDir: string): SnapshotBundle[] {
  if (!existsSync(snapshotsDir)) {
    return [];
  }
  ...
  for (const file of files) {
    try {
      ...
      const snapshot = JSON.parse(content) as SnapshotBundle;
...
function findSnapshot(snapshotsDir: string, nameOrAlias: string): SnapshotBundle | null {
...
export async function snapshotDiff(name: string, options: SnapshotDiffOptions = {}): Promise<void> {
  ...
  // Compare templates
  const templateDiffs: Array<{ name: string; status: 'added' | 'removed' | 'modified' | 'unchanged' }> = [];
  const templateFiles = ['chadgi-task.md', 'chadgi-generate-task.md'];

...
// Import shared types
import type { BaseCommandOptions, UpdateCheckCache, VersionInfo } from './types/index.js';
...
// Re-export VersionInfo for consumers
...
function readCache(): UpdateCheckCache | null {
  ...
  try {
    ...
    const cache: UpdateCheckCache = JSON.parse(cacheContent);
 ...
 */
function writeCache(latestVersion: string, currentVersion: string): void {
  const cachePath = getCachePath();
 ...
 */
function getChadGIVersion(): string {
 ...
 */
function getCommandVersion(command: string, versionArg: string = '--version'): DependencyInfo {
  try {
 ...
 */
function getClaudeVersion(): DependencyInfo {
 ...
 */
function getGhVersion(): DependencyInfo {
 ...
 * Get jq version
 */
function getJqVersion(): DependencyInfo {
 ...
 */
async function checkForUpdate(currentVersion: string, useCache: boolean): Promise<{
  available: boolean;
  latest: string;
  cached: boolean;
} | null> {
...
function printVersion(
  ...
  gh: DependencyInfo,
  jq: DependencyInfo,
  updateInfo: { available: boolean; latest: string; cached: boolean } | null
): void {
  ...
  if (updateInfo) {
    ...
    if (updateInfo.available) {
      console.log(
        `${colors.yellow}Update available:${colors.reset} ${updateInfo.latest} (current: ${currentVersion})`
      );
...
export async function version(options: VersionOptions = {}): Promise<void> {
  ...
  let updateInfo: { available: boolean; latest: string; cached: boolean } | null = null;
  if (options.check) {

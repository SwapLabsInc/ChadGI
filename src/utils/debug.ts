 ...
 */
export function setMaskSecretsInDebug(enabled: boolean): void {
 ...
 * ```
 */
export function traceLog(message: string, data?: unknown): void {
  if (!isTrace()) {
    return;
  }
 ...
 * ```
 */
export function debugDecision(decision: string, reason: string, context?: unknown): void {
  if (!isVerbose()) {
    return;
  }
 ...
 * ```
 */
export function debugFileOp(operation: string, path: string, details?: unknown): void {
  if (!isVerbose()) {
    return;
  }
 ...
 * ```
 */
export function traceApi(method: string, endpoint: string, requestData?: unknown): void {
  if (!isTrace()) {
    return;
  }
 ...
 * ```
 */
export function traceApiResponse(endpoint: string, responseData: unknown, durationMs?: number): void {
  if (!isTrace()) {
    return;
  }

...
const COMMAND_OPTIONS: Record<string, Array<{ short?: string; long: string; description: string; hasArg: boolean; values?: string[] }>> = {
  init: [
...
function generateBashCompletion(): string {
  ...
  const commandOptionsStr: string[] = [];
...
function generateZshCompletion(): string {
  ...
  const buildZshOptions = (opts: typeof COMMAND_OPTIONS[string]): string => {
    return opts.map(opt => {
...
function generateFishCompletion(): string {
  ...
  const commandCompletions = COMMANDS.map(c =>
    `complete -c chadgi -n "__fish_use_subcommand" -a "${c.name}" -d "${c.description}"`
  ).join('\n');

  // Build queue subcommand completions
  const queueCompletions = QUEUE_SUBCOMMANDS.map(c =>
    `complete -c chadgi -n "__fish_seen_subcommand_from queue; and not __fish_seen_subcommand_from ${QUEUE_SUBCOMMANDS.map(s => s.name).join(' ')}" -a "${c.name}" -d "${c.description}"`
  ).join('\n');

  // Build config subcommand completions
  const configCompletions = CONFIG_SUBCOMMANDS.map(c =>
    `complete -c chadgi -n "__fish_seen_subcommand_from config; and not __fish_seen_subcommand_from ${CONFIG_SUBCOMMANDS.map(s => s.name).join(' ')}" -a "${c.name}" -d "${c.description}"`
  ).join('\n');

  // Build completion subcommand completions
  const completionCompletions = COMPLETION_SUBCOMMANDS.map(c =>
    `complete -c chadgi -n "__fish_seen_subcommand_from completion; and not __fish_seen_subcommand_from ${COMPLETION_SUBCOMMANDS.map(s => s.name).join(' ')}" -a "${c.name}" -d "${c.description}"`
  ).join('\n');

  // Build option completions for each command
  const buildFishOptions = (cmd: string, opts: typeof COMMAND_OPTIONS[string]): string => {
    return opts.map(opt => {
      const parts: string[] = [`complete -c chadgi -n "__fish_seen_subcommand_from ${cmd.split(' ')[0]}"`];
